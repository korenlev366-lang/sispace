...
export function StatusBar({
  ...
  // status dot + word
  const statusColor = isBusy ? "yellow" : "green";
  const statusWord =
    overlay !== "chat"
      ? overlay // "sessions" | "model"
      : isBusy
        ? "busy"
        : "ready";
  ...
  // spec: amber for filled, faint for empty
  const leftEdge = "▕";
  const rightEdge = "▏";
  const filledBlock = "▎";
  const emptyBlock = "·";
  ...
  return (
    <Box flexDirection="row">
      ...
      <Text>
        ...
        <Text color="yellow">{filledBlock.repeat(filled)}</Text>
        <Text dimColor>
